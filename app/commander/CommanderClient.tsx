...
type FormuleType = "abonnement" | "pack" | "unique";
...
function getPrixUnitaire(tarifs: Tarif[], type: FormuleType, totalQty: number): number {
  const row = tarifs
    .filter((t) => t.type.toLowerCase() === type || (type === "unique" && ["unique","repas-unique","one-shot"].includes(t.type.toLowerCase())))
          ...
          <div key={s.key} className="flex items-center gap-0 flex-1 min-w-0">
            <div className="flex flex-col items-center shrink-0">
              <div
                ...
                style={{
                  background:   done ? "var(--green)" : active ? "var(--green)" : "#fff",
  ...
  return (
    <div className="flex items-center gap-2">
      <button
        ...
        style={{
          borderColor: value > min ? "var(--green)" : "var(--gray-200)",
          color:        value > min ? "var(--green)" : "var(--gray-300)",
          background:   "#fff",
        }}
      >
        −
      ...
      <button
        ...
        className="w-8 h-8 rounded-full border flex items-center justify-center text-lg font-bold transition-all duration-150"
        style={{
          borderColor: value < max ? "var(--green)" : "var(--gray-200)",
          color:        value < max ? "var(--green)" : "var(--gray-300)",
          background:   "#fff",
        }}
      >
        +
  ...
  return (
    <div
      ...
      style={{
        ...
        background:  active ? "rgba(74,103,65,.05)" : "#fff",
      }}
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <div>
          <p className="text-xs font-bold uppercase tracking-wider" style={{ color: active ? "var(--green)" : "var(--gray-400)" }}>
            {label}
          </p>
          <p className="text-sm font-medium leading-snug mt-0.5" style={{ color: "var(--dark)" }}>
            {platName}
          </p>
        </div>
      </div>
...
export default function CommanderClient({ menus, tarifs, points, activeWeek }: Props) {
  ...
  // Pre-select formule from URL param (?formule=abonnement etc.)
  useEffect(() => {
    const param = searchParams.get("formule") as FormuleType | null;
    if (param && ["abonnement", "pack", "unique"].includes(param)) {
      setFormule(param);
    }
  }, [searchParams]);
  ...
  function toggleJour(menu: Menu) {
    setSelections((prev) => {
      ...
      if (next.has(menu.id)) {
        ...
        next.set(menu.id, {
          menu_id: menu.id,
          date_livraison: menu.date_livraison,
          plat: menu.plat,
  ...
  function setQty(menuId: string, variant: "plat" | "vege", qty: number) {
    setSelections((prev) => {
      ...
      next.set(menuId, {
        ...cur,
        qty_plat: variant === "plat" ? qty : cur.qty_plat,
        qty_vege: variant === "vege" ? qty : cur.qty_vege,
      });
      return next;
    });
  }
  ...
  const FORMULES: { type: FormuleType; icon: string; label: string; desc: string; badge?: string }[] = [
    {
      type: "abonnement",
      icon: "🔄",
      label: "Abonnement",
      desc: "Commandez plusieurs jours par semaine. Tarif dégressif, le plus économique.",
      badge: "Populaire",
    },
    {
      type: "pack",
      icon: "📦",
      label: "Pack",
      desc: "Achetez un lot de repas à l'avance et profitez d'un meilleur prix au repas.",
    },
    {
      type: "unique",
      icon: "🍽️",
      label: "Repas unique",
      desc: "Commandez ponctuellement, sans engagement, selon vos envies du moment.",
    },
  ];
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <h1
            ...
            style={{ fontFamily: "'Playfair Display', serif", fontSize: "clamp(28px, 4vw, 38px)", color: "var(--dark)" }}
          >
        ...
        {/* Bandeau WhatsApp discret — toujours visible */}
        <div
          className="flex items-center gap-3 rounded-xl px-4 py-3 mb-6 text-sm"
          style={{ background: "var(--sand)", border: "1px solid rgba(0,0,0,.06)" }}
        >
          {/* WhatsApp icon */}
          ...
          <p style={{ color: "var(--gray-600)" }}>
            ...
            <a
              href="https://wa.me/33753791617?text=Bonjour%2C%20je%20souhaite%20commander%20avec%20Clodia%20%F0%9F%8D%BD%EF%B8%8F"
              target="_blank"
              rel="noopener noreferrer"
              className="font-semibold"
              style={{ color: "var(--green)" }}
            >
              Commandezvia notre bot →
            </a>
          </p>
        </div>
          ...
          <div>
            ...
            <div className="flex flex-col gap-4 mb-8">
              {FORMULES.map((f) => {
                ...
                // prix indicatif : premier tarif de ce type
                const tarif = tarifs.find((t) =>
                  t.type.toLowerCase() === f.type ||
                  (f.type === "unique" && ["unique","repas-unique","one-shot"].includes(t.type.toLowerCase()))
                );
                return (
                  <button
                    key={f.type}
                    type="button"
                    onClick={() => setFormule(f.type)}
                    ...
                    style={{
                      ...
                      background:  selected ? "rgba(74,103,65,.05)" : "#fff",
                    }}
                  >
                    {f.badge && (
                      <span
                        className="absolute -top-px right-5 px-2.5 py-0.5 rounded-b-lg text-xs font-bold"
                        ...
                        <div className="flex items-center justify-between gap-2">
                          ...
                          {tarif && (
                            <p className="text-sm font-bold shrink-0" style={{ color: "var(--green)" }}>
                              dès {tarif.prix_unitaire.toFixed(2).replace(".", ",")} €<span className="text-xs font-normal" style={{ color: "var(--gray-400)" }}>/repas</span>
                            </p>
                          )}
                        </div>
                        <p className="text-xs mt-1 leading-relaxed" style={{ color: "var(--gray-600)" }}>{f.desc}</p>
                      </div>
                      <div
            ...
            <button
              type="button"
              disabled={!formule}
              onClick={() => setStep("jours")}
              className="w-full py-3.5 rounded-xl text-sm font-semibold transition-all duration-200"
              style={{
                background:  formule ? "var(--green)" : "var(--gray-200)",
                color:       formule ? "#fff" : "var(--gray-400)",
                cursor:      formule ? "pointer" : "not-allowed",
              }}
            >
              Continuer →
            </button>
        ...
        {step === "jours" && (
          <div>
            <div className="flex items-center justify-between mb-1">
              <h2 className="text-base font-semibold" style={{ color: "var(--dark)" }}>Choisissez vos jours et vos plats</h2>
              <button
                type="button"
            ...
            {menus.length === 0 ? (
              <div className="bg-white rounded-xl p-8 border text-center" style={{ borderColor: "var(--gray-200)" }}>
                ...
                <p className="text-xs" style={{ color: "var(--gray-400)" }}>
                  Les menus de la prochaine semaine sont publiés chaque vendredi.
                </p>
              </div>
            ) : (
                ...
                {menus.map((menu) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div
                            ...
                            style={{
                              borderColor: checked ? "var(--green)" : "var(--gray-300)",
                              background:  checked ? "var(--green)" : "#fff",
                            }}
                          >
                            {checked && <span style={{ color: "#fff", fontSize: 9 }}>✓</span>}
                          </div>
                          <span className="text-sm font-semibold capitalize" style={{ color: "var(--dark)" }}>
                            {formatDate(menu.date_livraison)}
                          </span>
                        </div>
                        {menu.dessert && (
                      ...
                      {/* Variant cards — only shown when day is checked */}
                      {checked && sel && (
                        <div
                          className="px-4 pb-4 pt-2 border-t flex gap-3"
                          style={{ borderColor: "rgba(74,103,65,.12)" }}
                        >
                          <VariantCard
                            icon="🍽️"
                            label="Plat du jour"
                            platName={menu.plat}
                            qty={sel.qty_plat}
                            onChange={(v) => setQty(menu.id, "plat", v)}
                          />
                          <VariantCard
                            icon="🥗"
                            label="Option végé"
                            platName={menu.plat_vege}
                            qty={sel.qty_vege}
                            onChange={(v) => setQty(menu.id, "vege", v)}
                          />
                        </div>
                      )}
                    </div>
                ...
                <div>
                  ...
                  {prixUnitaire === 0 && (
                    <p className="text-xs mt-0.5" style={{ color: "var(--coral)" }}>
                      Tarif non configuré pour cette quantité
                    </p>
                  )}
                </div>
                <p className="text-lg font-bold" style={{ color: "var(--dark)" }}>
                  {totalPrix.toFixed(2).replace(".", ",")} €
                </p>
              </div>
            )}

            {/* Alerte si qty = 0 sur un jour sélectionné */}
            {Array.from(selections.values()).some((j) => j.qty_plat + j.qty_vege === 0) && (
              <p className="text-xs mb-3 px-1" style={{ color: "var(--coral)" }}>
                ⚠️ Chaque jour sélectionné doit avoir au moins 1 repas.
              </p>
            )}

            <button
              type="button"
              disabled={!canContinueJours || menus.length === 0}
              onClick={() => setStep("infos")}
              className="w-full py-3.5 rounded-xl text-sm font-semibold transition-all duration-200"
              style={{
                background:  canContinueJours ? "var(--green)" : "var(--gray-200)",
                color:       canContinueJours ? "#fff" : "var(--gray-400)",
                cursor:      canContinueJours ? "pointer" : "not-allowed",
              }}
            >
        ...
        {step === "infos" && (
          <div>
            <div className="flex items-center justify-between mb-5">
              ...
              <button type="button" onClick={() => setStep("jours")} className="text-xs font-medium" style={{ color: "var(--gray-400)" }}>
                ← Retour
              </button>
            </div>
            ...
            <div className="bg-white rounded-xl border mb-6 overflow-hidden" style={{ borderColor: "var(--gray-200)" }}>
              <div className="px-4 py-3 border-b" style={{ borderColor: "var(--gray-100)", background: "var(--gray-50)" }}>
                <p className="text-xs font-bold uppercase tracking-wider" style={{ color: "var(--gray-400)" }}>Récapitulatif</p>
              </div>
              <div className="divide-y" style={{ borderColor: "var(--gray-100)" }}>
                {Array.from(selections.values()).map((j) => (
                  <div key={j.menu_id} className="px-4 py-3">
                    <p className="text-xs font-semibold mb-1.5" style={{ color: "var(--gray-600)" }}>
                      {formatDate(j.date_livraison)}
                    </p>
                    <div className="flex flex-col gap-1">
                      {j.qty_plat > 0 && (
                        <div className="flex items-center justify-between">
                          <span className="text-xs" style={{ color: "var(--dark)" }}>🍽️ {j.plat} × {j.qty_plat}</span>
                          <span className="text-xs font-medium" style={{ color: "var(--dark)" }}>
                            {(j.qty_plat * prixUnitaire).toFixed(2)} €
                          </span>
                        </div>
                      )}
                      {j.qty_vege > 0 && (
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-xs font-medium" style={{ color: "var(--dark)" }}>
                            {(j.qty_vege * prixUnitaire).toFixed(2)} €
                          </span>
                        </div>
                      )}
                    </div>
                ...
                <div className="px-4 py-3 flex items-center justify-between">
                  <span className="text-sm font-bold" style={{ color: "var(--dark)" }}>Total</span>
            ...
            <div className="flex flex-col gap-4 mb-6">
              <div>
                <label className="text-xs font-semibold uppercase tracking-wider block mb-1.5" style={{ color: "var(--gray-600)" }}>
                  Email *
                </label>
                <input
                  type="email"
                ...
                <p className="text-xs mt-1" style={{ color: "var(--gray-400)" }}>
                  L&apos;email doit correspondre à votre compte Clodia.
                </p>
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  style={{
                    borderColor: "var(--gray-200)",
                    color:       pointId ? "var(--dark)" : "var(--gray-400)",
                    background:  "#fff",
                    cursor:      "pointer",
                  }}
                  onFocus={(e) => (e.currentTarget.style.borderColor = "var(--green)")}
            ...
            <button
              ...
              style={{
                background: loading ? "var(--gray-200)" : "var(--green)",
            ...
            >
              {loading ? (
                ...
                `Confirmer ma réservation · ${totalPrix.toFixed(2).replace(".", ",")} €`
              )}
            </button>
        ...
        {step === "done" && (
          <div className="text-center py-8">
            <div
              ...
              style={{ background: "rgba(74,103,65,.12)" }}
            >
              ✅
            </div>
            <h2
              className="font-bold mb-2"
              style={{ fontFamily: "'Playfair Display', serif", fontSize: "clamp(22px, 3vw, 28px)", color: "var(--dark)" }}
            >
              Réservation confirmée !
            </h2>
            <p className="text-sm leading-relaxed mb-6" style={{ color: "var(--gray-600)", maxWidth: 380, margin: "0 auto 24px" }}>
              Votre commande a bien été enregistrée. Vous recevrez une confirmation sur WhatsApp. Bon appétit ! 🍽️
            </p>
            ...
            {doneRef.length > 0 && (
              <div className="inline-flex flex-col gap-1 bg-white rounded-xl px-6 py-4 border mb-8" style={{ borderColor: "var(--gray-200)" }}>
                ...
                {doneRef.map((r) => (
                  <p key={r} className="font-mono text-sm font-bold" style={{ color: "var(--dark)" }}>{r}</p>
                ))}
              </div>
            )}

            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <Link href="/mon-compte" className="btn-primary px-8 py-3 text-sm font-semibold inline-block">
                Voir mes commandes
              </Link>
              <Link href="/" className="btn-secondary px-8 py-3 text-sm font-semibold inline-block">
                Retour à l&apos;accueil
              </Link>
            </div>
          </div>
